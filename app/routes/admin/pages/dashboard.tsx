...
import { getTripsByTravelStyle, getUserGrowthPerDay, getUsersAndTripsStats } from "~/modules/dashboard/lib/procedures";
import { getAllTrips } from "~/modules/trips/lib/procedures";
import { getAllUsers, getUser } from "~/modules/appwrite/auth";
        ...
        <div className="trip-grid">
          {allTrips.slice(0, 4).map((trip) => (
            <TripCard
              key={trip.id}
              id={`${trip.id}`}
              name={trip.name || "Anonymous"}
              imageUrl={trip.imageUrls[0]}
              location={trip.itinerary?.[0]?.location ?? ""}
              tags={[trip.interests || "Unknown", trip.travelStyle || "Unknown"]}
